...
calc_cor_of_cor <- function(ex_list){
  ...
  message('Computing intra-study correlations using ',
          length(genes), " genes")
...
#' @examples
#' \dontrun{
#' compare_networks(GSE39582_icwgcna$community_membership
...
compare_networks <- function(net_membership_1,
  ...
  comms_1    <- apply(net_membership_1, 1,
                      function(x){
                        x[x < memb_cut] <- 0
                        ind <- which(x == max(x))
                        if (length(ind) > 1) {ind <- NA}
                        return(ind)})
  comms_2    <- apply(net_membership_2, 1,
                      function(x){
                        x[x < memb_cut] <- 0;
                        ind <- which(x == max(x))
                        if (length(ind) > 1) {ind <- NA}
  ...
  net_membership_1 <- net_membership_1[inds,]; net_membership_1[net_membership_1 == 1] <- .99;
  net_membership_1[net_membership_1 == -1] <- -.99;
  net_membership_2 <- net_membership_2[inds,]; net_membership_2[net_membership_2 == 1] <- .99;
